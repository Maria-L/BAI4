		...
		while(running) {
			...
			for(MailKonto mailKonto : Server.mailKontoList()) {
				...
				try {
					...
					log.newWarning("Die Hostaddresse " + mailKonto.serverAddresse() + " konnte nicht aufgel�st werden");
					System.out.println("Die Hostaddresse " + mailKonto.serverAddresse() + " konnte nicht aufgel�st werden");
					continue run;
				} catch (IOException e) {
				...
				try {
					...
					for(int messageNum : availableMessages) {
						...
						if(inputFromServer.indexOf("+OK") != 0) {
							log.newWarning("Fehler beim L�schen von Nachricht Nummer " + messageNum + " vom Server " + mailKonto.serverAddresse());
							System.out.println("Fehler beim L�lschen von Nachricht Nummer " + messageNum + " vom Server " + mailKonto.serverAddresse());
							continue mailSchleife;
						}
					}
